...
export function printList(list, listName, action = true, command = "remove", hoverKey = false) {
    ...
    // Print out header
    const message = new Message("§8\n§8\n§8").setChatLineId(5858);
    const header = ChatLib.getCenteredText(`${listName}`);
    const whitespace = header.match(/^\s+/)[0];

    message.addTextComponent(whitespace);
    message.addTextComponent(` §d${convertToTitleCase(listName)} §8(§7Members: §f${length}§8)`);

    // Loop through variables
    if (length === 0) {
        message.addTextComponent(`\n` + ChatLib.getCenteredText(DARK_PURPLE + "  No members yet"));
    } else if (hoverKey) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            message.addTextComponent(`\n ${WHITE}● `);
            message.addTextComponent(
                new TextComponent(`${LIGHT_PURPLE + key}`)
                    .setClickAction("run_command")
                    .setClickValue(`/ka ${listName} ${command} ${key}`)
            ...
            if (action) {
                ...
                message.addTextComponent(
                    new TextComponent(`${YELLOW + list[i]}`)
                        .setClickAction("run_command")
                        .setClickValue(`/ka ${listName} ${command} ${list[i]}`)
            ...
            if (action) {
                ...
                message.addTextComponent(
                    new TextComponent(`${YELLOW + key}`)
                        .setClickAction("run_command")
                        .setClickValue(`/ka ${listName} remove ${key}`)
